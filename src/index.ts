	...
	async run(event: WorkflowEvent<ZillowCollectionParams>, step: WorkflowStep) {
		...
		const completedData = await step.do(
			...
			async () => {
				...
				const status = await statusResponse.json<BrightDataStatusResponse>();
		...
		// Step 3: Download the data from BrightData
		const rawZillowData = await step.do('download-zillow-data', async () => {
			console.log('Downloading data from:', completedData.url);
